 ...
 *  When this flag is set, CDATA markedsections are not passed literally to the @a passthrough function ofthe parser. Instead, the content of the section (without the`<![CDATA[` and `]]>`) ispassed to the @a text function. This flag was added in GLib 2.12.
 * 
 *  @var ParseFlags PREFIX_ERROR_POSITION
 *  Normally errors caught by GMarkupitself have line/column information prefixed to them to let thecaller know the location of the error. When this flag is set thelocation information is also prefixed to errors generated by theG::MarkupParser implementation functions.
 * 
 *  @var ParseFlags IGNORE_QUALIFIED
 *  Ignore (don't report) qualifiedattributes and tags, along with their contents.  A qualifiedattribute or tag is one that contains ':' in its name (ie: is inanother namespace).  @newin{2,40}
 * 
 *  @enum ParseFlags
 * 
