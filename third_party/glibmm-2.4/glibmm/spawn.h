 ...
 *  the first element of `argv` is the file toexecute, while the remaining elements are the actual argument vectorto pass to the file. Normally g_spawn_async_with_pipes() uses `argv[0]`as the file to execute, and passes all of `argv` to the child.
 * 
 *  @var SpawnFlags SPAWN_SEARCH_PATH_FROM_ENVP
 *  if `argv[0]` is not an abolute path,it will be looked for in the `PATH` from the passed child environment.@newin{2,34}
 * 
 *  @var SpawnFlags SPAWN_CLOEXEC_PIPES
 *  create all pipes with the `CLOEXEC` flag set.@newin{2,40}
 * 
 *  @enum SpawnFlags
 * 
